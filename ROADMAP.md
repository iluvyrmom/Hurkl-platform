...
- **Status note:** the Telegram channel built ahead of sequence (see Phase 4's status note) proves the message-routing pipeline end-to-end but uses `MockAIModelProvider`, not real reasoning — this phase's actual AI Router activation (a real `ANTHROPIC_API_KEY`, explicitly approved) is still what "text-based Mason" means here. As of 2026-08-06 this proof is real, not simulated: the founder messaged the live `M_Hurklbot` Telegram bot, deployed on Netlify (`hurkl-platform.netlify.app`), against the real `Hurkl-production` database, and received Mason's (mocked) reply through the actual webhook path — see `docs/communications-architecture.md`'s "Production incident, 2026-08-06" section for what broke and how it was fixed.
