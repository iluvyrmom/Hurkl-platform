...
- **Status note:** the Telegram channel built ahead of sequence (see Phase 4's status note) proves the message-routing pipeline end-to-end. As of 2026-08-06 the founder messaged the live `M_Hurklbot` Telegram bot, deployed on Netlify (`hurkl-platform.netlify.app`), against the real `Hurkl-production` database, and received a reply through the actual webhook path (at the time, `MockAIModelProvider`'s placeholder text — see `docs/communications-architecture.md`'s "Production incident, 2026-08-06" section for what broke on the way there and how it was fixed). As of 2026-08-10, real reasoning code (`AnthropicModelProvider`, tiered model routing, cost controls) is built, tested, and deployed, and a real `ANTHROPIC_API_KEY` is configured in production — but the Anthropic account has zero funded credits, so **no real reasoning request has succeeded yet**; see `docs/communications-architecture.md`'s "Real Mason reasoning" section. This phase is not complete until that live test actually succeeds. As of 2026-08-11, a hard server-side Anthropic spend-ceiling guard (`lib/mason/spend-guard.ts`, `lib/mason/budget-config.ts`) is also built, tested, and deployed ahead of billing being funded — see `docs/communications-architecture.md`'s "Hard Anthropic spend guard" section — and the v1 routing heuristic no longer escalates to the expensive tier on message length alone, only on genuine complexity signals.
...
- **Status note (2026-08-11):** the first slice of this phase — lead capture, ahead of sequence, same "pull forward what unblocks revenue" reasoning as Phase 4/6's Telegram work — is live in code: a real public page (`/book/a1-best-moving`), a real lead form, a real database record, visible to the owner on the dashboard. A-1 is seeded as a real company row with real contact info. Not yet done: the migration is not applied to production (Supabase access gap, not a code gap), A-1 has no real owner account attached yet, and quoting/booking/invoicing/payment/AI chat are explicitly deferred pending real pricing data and payment/AI provider funding — see `docs/a1-best-moving-launch.md` for the full record and exact owner actions required. This phase's MFA gate above still applies before any of this handles real customer payment data.
- **Status note (2026-08-13):** A-1's real pricing is now known (seeded as `companies.known_facts`) and the AI-chat gap above is closed in code — Mason's real conversation now replaces the structured form on `/book/a1-best-moving` whenever `ANTHROPIC_API_KEY` is configured, gathers what the form used to ask for, and submits a real lead. Still not done: production still needs the migrations applied and a funded/configured `ANTHROPIC_API_KEY` before any of this handles a real customer; deposit/payment collection is still blocked on connecting a real payment processor (no credentials exist); booking/scheduling and a firm itemized (vs. rate-card) estimate are still not built. See `docs/a1-best-moving-launch.md`'s 2026-08-13 update.
