...
## 1b. Foundational architectural decision: a Business Ecosystem Platform, not a CRM (founder-approved)

HURKL is fundamentally a **Business Ecosystem Platform**, not a traditional CRM. A CRM tracks a single company's own contacts and deals in isolation; Mason is built around one shared **Business Knowledge Graph** underlying every engine: businesses, projects, opportunities, trades, employees, equipment, certifications, compliance, customers, relationships, financial health, and operational readiness.

**No engine should duplicate research already performed elsewhere.** Every capability in `PRODUCT.md`'s "Growth & capacity management" section — the Opportunity Engine, Capacity Manager, Operations Compliance Engine (including Equipment Lifecycle and the Digital Project Compliance Binder), Financial Health Engine, Operational Readiness Engine, Relationship Value, Strategic Alliances, and the HURKL Trusted Business Network — is a different lens onto the same underlying graph, not a separate data silo holding its own duplicate copy of the same facts.
...
**Reporting hierarchy (permanent):** every specialist report passes through the Critical Review Specialist — the mandatory, universal gate that looks over everything before Mason ever sees it — plus whichever additional Quality Assurance Layer stages the report warrants (Evidence Verification, Risk, Conflict Resolution, Audit), before reaching Mason. Only Mason decides whether it can be handled within approved authority or must go to the owner. No specialist ever reports to the owner directly, and the owner never interacts with a specialist directly — the owner has exactly one employee: Mason. **When real specialist implementation begins, the Critical Review Specialist is the first one to build** — nothing reaches Mason unreviewed.
...
**Status note (2026-08-13):** a second channel, `web_public`, is now real — the first customer-facing one. It powers Mason's chat on a tenant's public booking page (`app/api/mason/chat/[slug]/route.ts`, `app/book/[slug]/mason-chat.tsx`), reuses the exact same `receiveMessage()` pipeline/system prompt/`AIModelProvider`/spend guard Telegram already used, and differs only in how its sender is resolved: anonymous, company resolved directly by public slug rather than a linked identity, plus a hard per-conversation turn cap as additional abuse protection a public unauthenticated endpoint needs that an internal, pre-authenticated channel doesn't. See `docs/a1-best-moving-launch.md`'s 2026-08-13 update for the full record.

### Future services (not yet designed in detail)

`PRODUCT.md` documents several growth-and-capacity capabilities that don't have services in the map above yet, because they aren't being built now: a **Capacity Manager**, an **Opportunity Engine** (including property/development signal ingestion and the Business Knowledge Graph), a **Business Maturity Advisor**, **Approved Outreach Playbooks**, **DEFERRD** (first-party physical fulfillment), an **Operations Compliance Engine** (including Equipment Lifecycle), a **Continuous Intelligence Engine**, a **Financial Health Engine**, **Strategic Alliances** support, a future **HURKL Trusted Business Network (HTBN)**, an **Operational Readiness Engine**, **Relationship Value**, and a **Business Risk Dashboard**. Voice-first capture ("Talk to Mason," see `PRODUCT.md`) is a nearer-term capture-UX capability, not a growth capability, but shares the same "not yet built" status. When these are eventually built:
...
- Dependency-free TypeScript domain contracts already exist ahead of implementation for nine of these: `lib/domain/fulfillment.ts` (DEFERRD), `lib/domain/compliance.ts` (Operations Compliance Engine, including the Digital Project Compliance Binder and Equipment Lifecycle — `SafetyCriticalEquipmentCategory`, `EquipmentAsset`, `EquipmentLifecycleEvent`), `lib/domain/evidence.ts` (Verified Intelligence / Evidence-Based Operations — `EvidenceSource`, `EvidenceRecord`, `RegulatoryRequirement`, `VerifiedFinding`, `RecommendationEvidence`, `Jurisdiction`, `VerificationStatus`, `SourceAuthorityLevel`, `ReverificationRule`), `lib/domain/finance.ts` (Financial Health Engine — `CashCategory`, `CashPosition`, `FinancialReviewCycle`, `TaxDeadline`, `FinancialDecisionForReview`, `AccountingPlatform`, `AccountingProvider`), `lib/domain/intelligence.ts` (Continuous Intelligence Engine), `lib/domain/alliances.ts` (Strategic Alliances — single-tenant relationship types only), `lib/domain/readiness.ts` (Operational Readiness Engine — `ReadinessCategory`, `ReadinessIssue`, `DailyReadinessReport`), `lib/domain/relationship-value.ts` (Relationship Value — `RelationshipValueFactor`, `CustomerRelationshipAssessment`), and `lib/domain/network-governance.ts` (HTBN governance — `NetworkMembershipStatus`, `OperationalEvidenceCategory`, `TenantOperationalEvidence`, `StewardshipReviewRecommendation`, `ImprovementPlan`, single-tenant perspective only). All nine are types only — no runtime logic, no database schema, no provider integration, no membership-decision logic, and specifically **no live web research crawler**, **no tax-strategy logic**, and **no speech-to-text or AI-extraction logic** — so later work has a stable shape to build against without re-deriving it.
- **The HTBN's cross-tenant membership-visibility mechanism is explicitly not covered by any domain contract.** `lib/domain/network-governance.ts` models only a tenant's own membership status and evidence — never a cross-tenant "is this business a member" lookup — because that would require a cross-tenant read beyond the single narrow exception §4 already documents. Per `CLAUDE.md`'s permanent rule, that exception must never be extended by precedent — any real cross-tenant design for HTBN is its own conscious, reviewed decision. See `docs/business-intelligence/TRUSTED_TRADE_NETWORK.md`.
...
- **Status note (2026-08-11):** a first real implementation exists for the Telegram channel — `lib/mason/task-classification.ts` (task classification), `lib/mason/model-routing.ts` (tier routing), `lib/mason/model-config.ts` (tier → real Anthropic model ID + pricing), `lib/mason/providers/anthropic-model-provider.ts` (the real `AIModelProvider`), and `lib/mason/spend-guard.ts`/`lib/mason/budget-config.ts` (a hard HURKL-wide + per-company daily/monthly spend ceiling, checked before every request, server-config only). This is not a claim that the full AI Router described below is complete — the spending limits that exist today are hard server-side ceilings (env vars), not owner-configurable per-tenant limits with pre-limit alerts and a slow-down/pause/notify-only choice; today there is one fixed routing policy, not a per-company configuration. See `docs/communications-architecture.md`'s "Real Mason reasoning" and "Hard Anthropic spend guard" sections.
