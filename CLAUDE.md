...
- **Mason's job title and external identity is Office Manager, not "Executive" or "COO" — those describe how he runs HAL internally, never a replacement identity (Knowledge Capture Session 010, `docs/business-intelligence/HAL_SPECIALIST_WORKFORCE.md`, `PRINCIPLES.md` 028).** Mason has his own real expertise (communication, office management, information synthesis, clear presentation, de-escalation, mentoring the owner, context/continuity, priority management, managing HAL) that no specialist ever performs on his behalf. Never implement Mason as a personality-free router that merely forwards requests to specialists — that failure mode is exactly as wrong as the "one giant all-knowing agent" failure mode HAL already forbids. Session 010 also added an operational service-delivery specialist family (lead qualification → estimating → calendar/capacity → booking → invoice → follow-up, gated by the Critic) alongside the existing growth/BD roster — see `lib/domain/hal.ts`'s `SpecialistType` and `ClaimVerificationClassification`.
