...
- `supabase/migrations/00000000000010_company_branding.sql` adds optional `logo_url`/`hero_eyebrow`/`hero_headline` columns to `companies` (additive, generic — any company can set these, not just A-1) and seeds A-1's with its real logo path and hero copy, matching the existing site's look.
