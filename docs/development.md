...
| `ANTHROPIC_*_SPEND_CEILING_USD` (four vars — see `lib/mason/budget-config.ts`) | `.env.local` (optional — conservative defaults apply when unset) | Not needed for ordinary CI | Site environment variables, only if the founder wants a non-default ceiling | Not applicable | — |
